...
const buildQueryOption = function ({
    ...
    for (let selection of selections) {
        ...
        if (!config && eagerHasMany) {
            if (hasManyConfig && hasManyConfig.outputStructure === 'Array' && (hasManyConfig.conditionFields == null || hasManyConfig.conditionFields.length === 0)) {
...
export default function (args: {
    ...
    selections = [...selections, ..._.union(attributes || [], (order || []).filter(o => typeof o[0] === 'string').map(o => o[0])).map(field => fieldToSelection(field))];
